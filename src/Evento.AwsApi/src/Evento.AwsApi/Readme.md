...
WORKDIR /src
COPY ["Evento.AwsApi.csproj", "Evento.AwsApi/"]
RUN dotnet restore "Evento.AwsApi/Evento.AwsApi.csproj"

WORKDIR "/src/Evento.AwsApi"
COPY . .
RUN dotnet build "Evento.AwsApi.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Evento.AwsApi.csproj" \
...
Execute unit tests
```
    cd "Evento.AwsApi/test/Evento.AwsApi.Tests"
    dotnet test
```

Deploy application
```
    cd "Evento.AwsApi/src/Evento.AwsApi"
